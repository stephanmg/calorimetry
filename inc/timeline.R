...
draw_day_night_rectangles <- function(df, p, light_start = 7, light_end = 19, light_offset = 0) {
   ...
   light_on <- TRUE
   color <- "grey"
   lapply(intervals, function(item) { p <<- p + annotate("rect", xmin = item - 12 + light_offset, xmax = item+light_offset, ymin=min(df$y), ymax=max(df$y), fill = color, alpha = 0.1); if (light_on) { color <<- "yellow"; light_on <<- FALSE }  else {  color <<- "grey"; light_on <<- TRUE}; })
