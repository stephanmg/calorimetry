...
sidebar_content <- sidebarPanel(
   ...
   div("In case of any detected inconsistency in the raw data, a warning is generated, and further analysis is postponed. Check the boxes to enable consistency checks for input data sets"),
   ...
   conditionalPanel(condition = "input.plot_type == 'RestingMetabolicRate'", sliderInput("window", "Window size", min = 1, max = 30, value = 1, step = 1)),
   ...
   checkboxInput("do_select_date_range", label = "Select experimental days"),
   conditionalPanel(condition = "input.do_select_date_range == true", uiOutput("daterange")),
   checkboxInput(inputId = "curate", label = "Trim data (start and end)"),
