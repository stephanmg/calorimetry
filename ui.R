...
page_for_data_import_example_data <- fluidPage(
   ...
   hr(),
   h4("IMPC data sets"),
   div(selectInput("impc_gene_symbol", "Gene symbol", choices=c("Ucp1", "Adipoq"), selected="Ucp1")),
   tags$script(HTML("
...
page_for_visualization <- fluidPage(
   ...
   selectInput(inputId = "ic_system", "Select indirect calorimetry platform", factor(c("General", "COSMED", "Sable", "CLAMS Oxymax", "CalR", "CaloPy"))),
   conditionalPanel(condition = "input.ic_system == 'General' || input.ic_system == 'CLAMS Oxymax' || input.ic_system == 'CalR' || input.ic_system == 'CaloPy'", selectInput("plot_type", "Select quantity to plot", factor(c("Metadata", "RawMeasurement", "TotalHeatProduction", "RestingMetabolicRate", "HeatProduction", "FuelOxidation")))),
