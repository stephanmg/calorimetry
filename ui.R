...
page_for_visualization <- fluidPage(
   ...
   selectInput(inputId = "ic_system", "Select indirect calorimetry platform", factor(c("General", "COSMED", "Sable", "CLAMS Oxymax", "CalR"))),
   conditionalPanel(condition = "input.ic_system == 'General' || input.ic_system == 'CLAMS Oxymax' || input.ic_system == 'CalR'", selectInput("plot_type", "Select quantity to plot", factor(c("Metadata", "RawMeasurement", "TotalHeatProduction", "RestingMetabolicRate", "HeatProduction", "FuelOxidation")))),
