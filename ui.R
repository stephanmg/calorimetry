   ...
   tabsetPanel(
      ...
      tabPanel("Main plot", 
         tagList(
            ...
            conditionalPanel("input.wavelet_plot_ui == true", checkboxInput("wavelet_detrend", "Detrend", value = TRUE)),
            conditionalPanel("input.wavelet_plot_ui == true", checkboxInput("wavelet_smooth", "Smooth rollmean", value = TRUE)),
            conditionalPanel("input.wavelet_plot_ui == true", checkboxInput("wavelet_interpolate", "Interpolate on regular time-grid", value = TRUE)),
            conditionalPanel("input.wavelet_interpolate == true && input.wavelet_plot_ui == true", numericInput("wavelet_interpolate_dt", "Delta t [h]", min = 0.1, max = 60, value = 0.25)),
            conditionalPanel("input.wavelet_plot_ui == true", numericInput("wavelet_upper_period", "Lower period [min]", min=0, max = 24*60, value = 1)),
            ...
            conditionalPanel("input.windowed_plot == true && input.plot_type != 'Metadata' && input.ic_system != 'Calobox'", h3("Windowed time-trace plot")),
            ...
            conditionalPanel("input.wavelet_plot_ui == true", plotlyOutput("waveletPlot")),
            conditionalPanel("input.wavelet_plot_ui == true", hr()),
            conditionalPanel("input.wavelet_plot_ui == true", plotlyOutput("waveletGlobalPlot")),
...
page_for_data_import_preprocessing <- fluidPage(
   ...
   checkboxInput(inputId="recalculate_HP", "Re-calculate HP", value = TRUE),
...
page_for_visualization <- fluidPage(
   ...
   tabsetPanel(id = "tabsPC", type = "hidden",
      tabPanelBody("PC",
   selectInput(inputId = "ic_system", "Select indirect calorimetry platform", factor(c("General", "COSMED", "Sable", "Calobox"))),
   conditionalPanel(condition = "input.ic_system == 'General' || input.ic_system == 'Calobox'", selectInput("plot_type", "Select quantity to plot", factor(c("Metadata", "RawMeasurement", "TotalHeatProduction", "RestingMetabolicRate", "HeatProduction", "FuelOxidation")))),
   ...
   selectInput("time_scale_for_plot", "Select time-scale", choices=c("h", "s"), selected="h"),
))),
