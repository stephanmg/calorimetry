...
page_for_visualization <- fluidPage(
   ...
   tabsetPanel(id = "tabsPC", type = "hidden",
      tabPanelBody("PC",
   selectInput(inputId = "ic_system", "Select indirect calorimetry platform", factor(c("General", "COSMED", "Sable", "CaloPy"))),
   conditionalPanel(condition = "input.ic_system == 'General' || input.ic_system == 'CaloPy'", selectInput("plot_type", "Select quantity to plot", factor(c("Metadata", "RawMeasurement", "TotalHeatProduction", "RestingMetabolicRate", "HeatProduction", "FuelOxidation")))),
