...
page_for_data_import <- fluidPage(
   ...
   checkboxInput("upload_data_folder", "Upload folder instead?", value=FALSE),
   conditionalPanel("input.upload_data_folder == false", numericInput("nFiles", "Number of cohorts/subjects", value = 1, min = 1, step = 1)),

   conditionalPanel("input.upload_data_folder == true",
  tags$input(
    id = "folder", 
    type = "file", 
    webkitdirectory = NA, 
...
page_for_visualization <- fluidPage(
   ...
   selectInput("plot_type", "Select quantity to plot", factor(c("Metadata", "RawMeasurement", "TotalHeatProduction", "RestingMetabolicRate", "HeatProduction", "FuelOxidation"))),
