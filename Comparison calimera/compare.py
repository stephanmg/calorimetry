...
if __name__ == "__main__":
    ...
    dfCalimera = pd.read_csv('comp_table.tsv', sep='\t')
    ...
    min_RMRs = []
    for index, animal in enumerate(animal_ids):
        min_RMRs.append(24 * 6 * min(dfShiny.loc[dfShiny["Animal"] == int(animal)]["HP"].tolist())) 

    min_RMRsRef = []
    for index, animal in enumerate(animal_ids):
        min_RMRsRef.append(24 * 6 * min(dfCalimera[animal][1:].tolist()))
